...
- **Session folder:** `data/sessions/[dd.mm]/Remote/`.
...
- Save to `data/sessions/[dd.mm]/[Branch] CV.docx` (or `General CV.docx`). No country subfolder.
...
- Save to `data/sessions/[dd.mm]/Requests/`.

## `Run Blacklist: add [Company], [Company]`
## `Run Blacklist: remove [Company], [Company]`

Edit `data/Blacklist.txt`. Confirm the change. No other pipeline steps run.

## `Run Interview Prep: [Company], [Job Title]`

Invoke the `interview-prep` skill. Pull the JD from the job log's Apply Link if the role is logged, else search. Produce the prep document in `data/interview-prep/`.

## `Run Story Bank Refresh`

Invoke the `story-bank` skill's refresh protocol. Extract new STAR+R stories from the career file into `data/Interview Story Bank.txt`. No duplicates, no invention.
