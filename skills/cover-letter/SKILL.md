...
3. **Address the obvious objection, sourced from the diagnosis.** Read the diagnosis's `## Positioning` section before drafting: its mode and rationale ARE the objection call. `transition` and `adjacent` candidacies get an objection paragraph built from that rationale (the title gap, the career change) — named in one honest sentence, resolved in one. `direct` skips it unless the rationale names another live risk. Never re-derive the objection from the CV: the diagnosis already made the call, and a letter that contradicts the CV's positioning is worse than either alone. The recruiter is looking for that risk anyway; a candidate who names it first reads as confident, not weak. See [`references/objections-and-close.md`](./references/objections-and-close.md).
